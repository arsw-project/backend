...
import {Difficulty, Ticket, TicketStatus} from '@tickets/domain/entities/ticket.entity';
...
export class TicketDrizzleAdapter implements TicketRepository {
    ...
    async updateAssignee(id: string, assigneeId: string | null): Promise<Ticket | null> {
        return this.updateTicket(id, {assigneeId});
    }
    async updateDifficulty(id: string, difficulty: Difficulty): Promise<Ticket | null> {
        return this.updateTicket(id, {difficulty});
    }
