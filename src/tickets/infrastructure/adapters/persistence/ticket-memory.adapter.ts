...
export class TicketMemoryAdapter extends TicketRepository {
    ...
    findAllByOrgId(orgId: string): Promise<Ticket[]> {
        return Promise.resolve(this.tickets.filter((ticket) => ticket.orgId === orgId).map(this.cloneTicket));
    }

    findByAssigneeId(assigneeId: string): Promise<Ticket[]> {
        return Promise.resolve(
            this.tickets.filter((ticket) => ticket.assigneeId === assigneeId).map(this.cloneTicket),
        );
    }

    findByTag(tag: string): Promise<Ticket[]> {
        return Promise.resolve(
            this.tickets.filter((ticket) => ticket.tags.includes(tag)).map(this.cloneTicket),
        );
    }

    findByCreatorId(creatorId: string): Promise<Ticket[]> {
        return Promise.resolve(
            this.tickets.filter((ticket) => ticket.createdBy === creatorId).map(this.cloneTicket),
        );
    }

    findByStatus(status: TicketStatus): Promise<Ticket[]> {
        return Promise.resolve(
            this.tickets.filter((ticket) => ticket.status === status).map(this.cloneTicket),
        );
    }

    findByDifficulty(difficulty: Difficulty): Promise<Ticket[]> {
        return Promise.resolve(
            this.tickets.filter((ticket) => ticket.difficulty === difficulty).map(this.cloneTicket),
        );
    }
    ...
    updateAssignee(id: string, assigneeId: string | null): Promise<Ticket | null> {
    ...
    private applyUpdate(
        id: string,
        updates: Partial<Pick<Ticket, 'status' | 'assigneeId' | 'difficulty'>>,
    ): Promise<Ticket | null> {
        const index = this.tickets.findIndex((ticket) => ticket.id === id);
        if (index === -1) {
